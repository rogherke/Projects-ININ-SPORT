...
import "./cla_checkout2.scss";
import $ from "jquery";

class ClaCheckout3 extends Component {
  constructor(props) {
    super(props);
  ...
  handleprop = evt => {
    ...
    var b = JSON.parse(localStorage.getItem("cla"))
    var e  = new Date()
    fetch("http://localhost:3000/join/join", {
      method: "POST",
      body: JSON.stringify({
    ...
    localStorage.setItem("coaName", JSON.stringify(b[0].memName))
    localStorage.removeItem("cla")
    window.location.assign("http://localhost:3001/store/cla_checkout3");
  };
